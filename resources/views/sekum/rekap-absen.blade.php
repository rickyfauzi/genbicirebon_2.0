                ...
                <form action="rekap-absen-result" method="GET" id="form_id">
                    <div class="form-group">
                        <select class="form-control select  @error('kegiatan') is-invalid @enderror" name="id_kegiatan" onChange="document.getElementById('form_id').submit();">
                            ...
                            @foreach ($sekumKegiatan as $key=>$list)
                            <!-- <option value="{{ $list->id }}" {{ $list->id == $list->id ? "selected" : $list->id}}>{{ $list->jenis }} - {{ $list->nama_kegiatan }}</option> -->
                        ...
                        <div class="card-body">
                            <div class="page-header">
                                <div class="row align-items-center">
                                    <div class="col">
                                        <h3 class="page-title">
                                            @if(Route::is('sekum/rekap-absen'))    
                                            {{ $sekumAbsenKeg }}
                                            @else
                                            {{ $sekumAbsenKeg->nama_kegiatan }}
                                            @endif
                                        </h3>
                                    </div>
                            ...
                            <div class="table-responsive">
                                <table
                                    ...
                                    <tbody>
                                        @foreach ($sekumAbsen as $key=>$list )
                                        <tr>
