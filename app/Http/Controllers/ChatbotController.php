...
class ChatbotController extends Controller
{
    ...
    public function sendMessage(Request $request)
    {
        ...
        try {
                ...
                if ($firestoreAnswer && !empty(trim($firestoreAnswer))) {
                    ...
                    if (isset($openAIResult['answer']) && !empty(trim($openAIResult['answer']))) {
                        ...
                        if ($contextData === null && $this->shouldSaveToKnowledgeBase($message, $openAIResult['answer'])) {
                            Log::info("Mencoba menyimpan ke knowledge base...");
                            ...
                            $reasons = [];
                            if ($contextData !== null) $reasons[] = "has context data";
                            if (!$this->shouldSaveToKnowledgeBase($message, $openAIResult['answer'])) $reasons[] = "should not save";
                            Log::info("Tidak menyimpan ke knowledge base karena: " . implode(", ", $reasons));
                        }
                    } else {
            ...
            // Simpan log percakapan dan update metrics
            if ($source !== 'error') {
                Log::info("Menyimpan chat log dengan source: {$source}");
                $logSaved = $this->firestoreService->addChatLog($sessionId, $message, $response['message'], $source, $userId);
                if ($logSaved) {
                    Log::info("✅ Chat log berhasil disimpan");
                } else {
            ...
            try {
                $errorSaved = $this->firestoreService->addErrorLog($e->getMessage(), $message);
                if ($errorSaved) {
                    Log::info("✅ Error log berhasil disimpan");
                } else {
                    Log::warning("❌ Gagal menyimpan error log");
                }
    ...
    private function isDialogflowResponseValid($dialogflowResponse): bool
    {
        if ($dialogflowResponse === null) {
            Log::info("Dialogflow response is null");
            return false;
        }

        if (!isset($dialogflowResponse['text']) || empty(trim($dialogflowResponse['text']))) {
            Log::info("Dialogflow response text is empty");
            return false;
        }

        if (!isset($dialogflowResponse['is_fallback']) || $dialogflowResponse['is_fallback']) {
            Log::info("Dialogflow response is fallback: " . ($dialogflowResponse['is_fallback'] ?? 'unknown'));
            return false;
        }

        if ($this->isGenericResponse($dialogflowResponse['text'])) {
            Log::info("Dialogflow response is generic: " . $dialogflowResponse['text']);
            return false;
        }

        Log::info("Dialogflow response is VALID: " . $dialogflowResponse['text']);
        return true;
    }
